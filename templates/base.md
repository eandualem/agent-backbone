...
this backbone injects at every fresh start, selected by your tags; they are
maintained by {policy_maintainer}, outside any repository. Follow them; do not
restate them in your own instructions. To change one, or when you notice a
practice your whole group should share, propose it to {policy_maintainer} (an
issue, or `backbone tell` when the maintainer is an agent) — never by copying
the rule into a repository's instructions or your own configuration.
