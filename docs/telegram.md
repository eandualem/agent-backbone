...
- **Refused** — `🚫 Refused — grace`, when Claude's auto-mode safety check refuses a tool call on its own: no dialog appears and the agent keeps working, so nothing else would show it. The alert names the action safely (the program and subcommand, such as `gh issue edit`, or the browser actions, such as `claude-in-chrome: browser_batch (computer:left_click)`; never arguments, paths, URLs or page text) and the classifier's category (`External System Writes`). It has no buttons: there is no remote approval for a classifier refusal. To allow it, use `/permissions` in the agent's terminal or do it yourself, then tell the agent to continue. Once per agent and action within `timing.escalation_dedup_seconds`; refusals from before the service started are never replayed, and the backbone never retries a refused action. Sessions started before the upgrade get the hook at their next start.
...
- **Agent is offline with N queued messages** — messages are waiting for an agent that is not running (agents without `always_on`, which were not reported when they died, and not inbox-only; once per `timing.escalation_dedup_seconds`); it was not restarted.
