...
- **Refused** — `🚫 Refused — app`, when Claude's auto-mode safety check or Codex's automatic reviewer (`agents.auto_review`) refuses a tool call on its own: no dialog appears and the agent keeps working, so nothing else would show it. The alert names the action safely (the program and subcommand, such as `gh issue edit`, or the browser actions, such as `claude-in-chrome: browser_batch (computer:left_click)`; never arguments, paths, URLs or page text) and the reason's category: the classifier's (`External System Writes`), or the reviewer's risk level (`risk: high`) or `review timed out`. It has no buttons: there is no remote approval for these refusals. To allow it, use `/permissions` (Claude Code) or `/approve` (Codex) in the agent's terminal or do it yourself, then tell the agent to continue. Once per agent and action within `timing.escalation_dedup_seconds`; refusals from before the service started are never replayed, and the backbone never retries a refused action. Sessions started before the upgrade get the hook at their next start.
