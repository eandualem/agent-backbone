...
| `agent output [NAME] [--lines N] [--since CURSOR] [--screen] [--json]` | What the agent has been doing, **reads only**: a bounded tail of the runtime's own transcript for Claude Code (`~/.claude/projects/*/<session>.jsonl`) and Codex (`~/.codex/sessions/**/rollout-*<session>.jsonl`), located from the session id its hook recorded — user prompts, replies, tool calls and the first line of each result, thinking skipped, one clipped line each. Default 40 entries, at most 500, read from the end of the file (never a whole session). The printed `cursor` continues a later call with `--since`. Other runtimes, `--screen`, or a missing transcript show the visible terminal instead, ANSI stripped; the output always says which (`transcript` or `screen`). Transcript entries include activity that never reached the screen. `NAME` defaults to `$BACKBONE_AGENT`; works through the API, or directly when the backbone is down |
