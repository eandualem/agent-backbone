...
| Agent output (`agent output`, `GET /api/sessions/{name}/output`) | Read-only, registered agents only; tails the runtime's own transcript file for Claude Code and Codex (prompts, replies, tool calls, first result lines — activity beyond what is on screen), bounded to recent entries, else the visible screen | — |
