...
By default, supported runtimes receive a short brief when a new agent session starts,
so agents can start other agents, subscribe to repositories, message each other and
form teams without a person in the loop. Plain shells receive no brief, and resumed
sessions are not re-briefed where that would alter the conversation. You can watch
any session at any time, or step in.
