...
from agent_backbone.services.github import GitHubClient, review_started_event, review_status_event
...
async def _handle(
    ...
    if event_type_str == "status":
        reviewer = (payload.get("sender") or {}).get("login", "")
        if (
            gh is None
            or payload.get("state") != "pending"
            or not config.github.is_reviewer(reviewer)
        ):
            return Response(content="ignored: status is not a pending configured review")
        repo = payload.get("repository", {}).get("full_name", "")
        sha = payload.get("sha", "")
        outcomes = []
        for pull in await gh.list_pulls_raw(repo):
            if (pull.get("head") or {}).get("sha") != sha:
                continue
            event = review_status_event(payload, sha, pull, repo, delivery_id)
            if event is not None:
                outcomes.append(await dispatch_event(event, config, db, gh))
        return Response(content="; ".join(outcomes) or "ignored: status has no current open PR")
