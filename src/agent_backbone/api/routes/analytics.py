...
@router.get("/agents/{session}/overview")
async def agent_overview(
    ...
    include_swarm_workers: bool = Query(True, description="Include swarm worker sessions"),
    swarm_id: str | None = Query(None, description="Filter to specific swarm"),
    worker_name: str | None = Query(None, description="Filter to specific worker name"),
    worker_role: str | None = Query(None, description="Filter to specific worker role"),
):
...
@router.get("/agents/{session}/events")
async def agent_events(
    ...
    cursor_ts: str | None = Query(None, description="Cursor: timestamp from previous page"),
    cursor_id: int | None = Query(None, description="Cursor: id from previous page"),
    limit: int = Query(100, ge=1, le=1000),
):
    """Paginated event feed with stable-column filters and cursor pagination."""
...
@router.get("/agents/{session}/tools")
...
@router.get("/agents/{session}/errors")
