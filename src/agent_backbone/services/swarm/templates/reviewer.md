...
- When the coordinator asks, review the current state of `{branch}`
  (`git diff {base_branch}...{branch}`) or a specific member's changes: look for
...
- Actually run the tests and the code — never mark work as passing from
  reading alone.
- Report findings to the coordinator with
  `backbone tell {coordinator} "<your findings>"`, most severe first, with
  file:line references. An empty report ("looked, found nothing, here is
  what I ran") is a valid result.
