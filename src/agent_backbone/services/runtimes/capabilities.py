...
``implemented`` optionally names the adapter attribute behind a row: the
registry test fails when a cell claims ``supported`` for a runtime whose
adapter lacks it.
"""
...
def _ok(note: str = "") -> Cell:
    return Cell("supported", note=note)
...
def _row(
    ...
    *,
    ...
    return Capability(id, title, cells, implemented)
...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    _row(
        ...
        claude=_ok("with `backbone chrome install`"),
        codex=_gap(270),
        gemini=_gap(270),
        opencode=_gap(270),
        deepcode=_gap(270),
