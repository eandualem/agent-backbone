...
class TestCreateAndNotify:
    async def test_creates_issue_and_notifies_target(self, config):
        ...
        with (
            patch(
                "agent_backbone.services.delivery._create_notify.resolve_entity_session",
                new_callable=AsyncMock,
                return_value="brunel",
            ) as mock_resolve,
            patch(
                "agent_backbone.services.delivery._create_notify.safe_deliver",
                new_callable=AsyncMock,
    ...
    async def test_notification_uses_format_issue_notification(self, config):
        ...
        from agent_backbone.services.notifications import format_issue_notification
