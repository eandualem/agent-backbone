...
def _join_patches(mgr, panes=None, resize_ok=True, win_size=(200, 50)):
    ...
    return (
        ...
        patch("api.socketio_server.list_panes", new_callable=AsyncMock, return_value=panes),
        patch("api.socketio_server.resize_window", new_callable=AsyncMock, return_value=resize_ok),
        patch("api.socketio_server.get_window_size", new_callable=AsyncMock, return_value=win_size),
    )
...
class TestOrphanGuard:
    ...
    async def test_error_after_pty_creation_cleans_up(self):
        ...
        with (
            patch(
                "api.socketio_server.session_exists",
                new_callable=AsyncMock,
                return_value=True,
            ),
            patch("api.socketio_server.get_pty_manager", return_value=mgr),
            patch("api.socketio_server.list_panes", new_callable=AsyncMock, return_value=[]),
            patch(
                "api.socketio_server.resize_window",
                new_callable=AsyncMock,
...
class TestResizeFailureLogging:
    ...
    async def test_join_logs_resize_failure(self, caplog):
        ...
        with (
            patch("api.socketio_server.session_exists", new_callable=AsyncMock, return_value=True),
            patch("api.socketio_server.get_pty_manager", return_value=mgr),
            patch("api.socketio_server.resize_window", new_callable=AsyncMock, return_value=False),
            patch(
                "api.socketio_server.get_window_size",
                new_callable=AsyncMock,
                return_value=(200, 50),
            ...
            with caplog.at_level(logging.ERROR, logger="api.socketio_server"):
                await ns.on_join("sid1", {"session": "ike", "cols": 160, "rows": 35})
                await asyncio.sleep(0.1)
...
class TestPtyManagerOrphanCleanupPidFile:
    ...
    def test_init_reads_pid_file_and_kills(self, tmp_path):
        ...
        pty_mod = "agent_backbone.services.streaming._pty"
        with (
