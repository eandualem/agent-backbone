...
class TestCreateRoom:
    ...
    async def test_create_room_success(self, api_client, auth_headers, room_dir, api_app):
        ...
        try:
            with patch(
                "api.routes.rooms._inject_meeting_skill",
...
class TestMeetingSkillInjection:
    ...
    def test_read_meeting_skill_success(self, tmp_path):
        ...
        with patch("api.routes.rooms._MEETING_SKILL_PATH", skill_file):
    ...
    def test_read_meeting_skill_missing(self, tmp_path):
        ...
        with patch("api.routes.rooms._MEETING_SKILL_PATH", missing):
            result = _read_meeting_skill()
    ...
    async def test_inject_meeting_skill_delivers_to_all(self):
        ...
        with (
            patch(
                "api.routes.rooms._read_meeting_skill",
...
class TestCursorAdvancement:
    ...
    async def test_skill_injection_sets_initial_cursors(self):
        ...
        with (
            ...
            patch("api.routes.rooms._save_room", new_callable=AsyncMock) as mock_save,
        ):
            await _inject_meeting_skill(room, config, delivery_svc)
